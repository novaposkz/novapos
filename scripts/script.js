...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateValues(slider, price) {
      const container = slider.closest('.calculator-option');
      ...
      const monthlyAmount = value * price;
      const yearlyAmount = monthlyAmount * 12;

      quantity.textContent = value;
      monthly.textContent = formatNumber(monthlyAmount);
      yearly.textContent = formatNumber(yearlyAmount);
  ...
  function updateTotal() {
      ...
      sliders.forEach((slider) => {
          ...
          if (id === 'licenses-slider') price = prices.licenses;
          else if (id === 'equipment-slider') price = prices.equipment;
          else if (id === 'support-slider') price = prices.support;

          totalYearly += value * price * 12;
      });

      document.getElementById('total-income').textContent = formatNumber(totalYearly);
  }
  ...
  document.querySelectorAll('.slider').forEach((slider) => {
      ...
      slider.addEventListener('input', function () {
          updateValues(this, price);
      });

      // Initialize values
      updateValues(slider, price);
  });
});